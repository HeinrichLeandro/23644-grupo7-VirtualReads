        ...
        <div key={index} className={`card mb-3 tarjeta${index + 1}`} >
          <div className="row g-0 tarjHome">
            <div className="col-md-4">
              <Link to={`/books/${index + 1}`}><img src={libro.volumeInfo.imageLinks?.thumbnail || 'URL_IMAGEN_POR_DEFECTO'} className="img-fluid rounded-start imgLibro" alt={libro.volumeInfo.title} onClick={() => onCardClick(libro)} /></Link>
            </div>
            <div className="col-md-4">
              <div className="card-body text-start">
                <Link to={`/books/${index + 1}`}><h5 className="card-title" onClick={() => onCardClick(libro)}>{libro.volumeInfo.title}</h5></Link>
